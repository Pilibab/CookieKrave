...
import type { Order, BOMEntry, InventoryItem } from "@/types/mytypes";

type OrderStatus = "Pending" | "Baked" | "Out for Delivery" | "Completed" | "Cancelled";

const STATUS_OPTIONS: OrderStatus[] = ["Pending", "Baked", "Out for Delivery", "Completed", "Cancelled"];

const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  Pending:           { bg: "rgba(254,243,199,0.12)", text: "#fef08a",  border: "#fef08a22" },
  Baked:             { bg: "rgba(254,215,170,0.12)", text: "#fdba74",  border: "#fdba7422" },
      ...
      if (nextStatus === "Completed" && currentOrder.order_status !== "Completed") {
        ...
        for (const productId of productIds) {
          ...
          for (const entry of bomEntries) {
            ...
            const deductQty = entry.quantity ?? (entry as any).bom_quantity ?? 0;
